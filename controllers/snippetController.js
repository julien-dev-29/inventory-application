...
const validateSnippet = [
  body("title")
    .trim()
    .isLength({ min: 2 })
    .withMessage("Nom " + lengthMinErr)
    .isLength({ max: 10 })
    .withMessage("Nom " + lengthMaxErr),
  body("description")
    .trim()
    .isLength({ min: 8 })
    .withMessage("Description " + lengthMinDescErr),
];
...
export default {
  ...
  index: async (req, res) => {
    ...
    res.render("snippets/index", {
      ...
      snippets:
        await snippetRepository.getPaginatedSnippetsWithLanguageAndProjectsAndTags(
          offset,
        ),
  ...
  details: async (req, res) => {
    res.render("snippets/details", {
      ...
      title: "Details",
      snippet:
        await snippetRepository.getSnippetByIdWithLanguageAndProjectsAndTags(
          req.params.id,
        ),
      slug: "snippets",
    });
  ...
  update: [
    ...
    async (req, res) => {
      ...
      snippetRepository
        .updateSnippetById(req.params.id, req.body)
